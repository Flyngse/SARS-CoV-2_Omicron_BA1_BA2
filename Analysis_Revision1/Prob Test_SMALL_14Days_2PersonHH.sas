...
proc genmod data = data.Analysis_daily plots=none ;
	where variant_index in ("Omicron - BA.1", "Omicron - BA.2")  and house_members = 2;
	by variant_index ;
	class 
			house_number 
			time_from_index
...
ods graphics / reset=index imagename = "ProbTest_small_14Days_2PersonHH" imagefmt=pdf ;
...
ods listing gpath = "F:\Projekter\FSEID00004942\FrederikPlesnerLyngse\Omicron_BA1_BA2\Output_Revision1\" ;
ods graphics / reset=index imagename = "ProbTestPos_small_14Days_2PersonHH" imagefmt=pdf ;
ods graphics on / attrpriority=none border=off ;
Title ;
proc sgplot data = parms ;
	where Days > 0 ;
